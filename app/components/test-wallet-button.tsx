...
export function TestWalletButton() {
  ...
  useEffect(() => {
    ...
    const mockWalletConnected = localStorage.getItem('mockWalletConnected') === 'true'
  ...
  const injectMockWallet = () => {
    ...
    if (typeof window !== 'undefined') {
      (window as any).mockSolana = {
        isPhantom: true,
        publicKey: new PublicKey(TEST_WALLET_PUBLIC_KEY),
        isConnected: true,
        connect: async () => {
          return { publicKey: new PublicKey(TEST_WALLET_PUBLIC_KEY) }
        },
        disconnect: async () => {
      ...
      window.dispatchEvent(new CustomEvent('mock-wallet-ready', {
