...
import { getRpcUrl } from "./utils/test-helpers";
...
test.describe("Hub Management", () => {
  ...
  test("should connect wallet using dev wallet", async ({
    page,
  }: {
    page: Page;
  }) => {
  ...
  test("should handle provider addition flow", async ({
    page,
    context,
  }: {
    page: Page;
  ...
  test("should fetch hub data correctly", async ({ page }: { page: Page }) => {
    // Use the RPC client to verify hub data matches what's displayed
    const rpc = createSolanaRpc(testConfig.rpcUrl);
    const hubPDA = address(testConfig.hubPDA);

    // Check if hub account exists
    const hubAccount = await rpc.getAccountInfo(hubPDA).send();
    expect(hubAccount.value).toBeTruthy();

    // Verify the hub owner is our program
    expect(hubAccount.value!.owner).toBe(testConfig.programId);
...
test.describe("Direct Blockchain Interaction", () => {
  test("should be able to query hub directly", async () => {
    ...
    const balanceResponse = await rpc
      .getBalance(address(keypair.publicKey.toString()))
      .send();
    const balance = balanceResponse.value;
    console.log(`Hub authority balance: ${Number(balance) / 1e9} SOL`);
    expect(Number(balance)).toBeGreaterThan(0);

    // Verify hub PDA exists
    const hubPDA = address(testConfig.hubPDA);
    const hubInfo = await rpc.getAccountInfo(hubPDA).send();
    expect(hubInfo.value).toBeTruthy();
    expect(hubInfo.value!.owner).toBe(testConfig.programId);
  });
});
